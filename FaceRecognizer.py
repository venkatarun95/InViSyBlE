...
class FaceRecognizer(Computation):
	dependancies = [GetBWFrame, SIFTKeypoints, FaceDetector]
	...
	def run(self, params):
		...
		kpts, descrs = params[1][0], params[1][1] #sift.detectAndCompute(frame[0], None)
		candidates = {}

		#print params[1]
		detectedFaces = params[2]
		recognizedFaces = []

		for face in detectedFaces:
			x,y,w,h = face
			for kpId in range(len(descrs)):
				if x < kpts[kpId].pt[0] < x+w and y < kpts[kpId].pt[1] < y+h:
					descr, kp = descrs[kpId], kpts[kpId]
			...
			recognizedFaces += [detected]
		return recognizedFaces #labels[detected]